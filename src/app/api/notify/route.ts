...
export async function POST(req: Request) {
  try {
    ...
    const { record } = body; // Supabase webhook envía el registro en 'record'

    if (!record) {
      return NextResponse.json({ error: 'No record found' }, { status: 400 });
    }

    const adminEmail = process.env.ADMIN_EMAIL || 'sergiotg.dev@gmail.com'; // Cambiar por tu email real
    ...
    const emailRes = await resend.emails.send({
      ...
      to: adminEmail,
      subject: `🔔 Nuevo Socio: ${record.nombre_complejo}`,
      html: `
        <h1>Nuevo registro de alojamiento</h1>
        ...
        <br>
        <p>Revisa todos los detalles en tu panel de Supabase.</p>
      `,
    });

    // 2. Enviar Notificación a Telegram (Opcional si están las keys)
    if (telegramBotToken && telegramChatId) {
      const message = `🔔 *NUEVO SOCIO REGISTRADO*\n\n` +
